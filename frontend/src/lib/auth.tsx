import { clearWorkerNotes } from './worker-offline'
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const validateServerSession = async () => {
      ...
      if ((error && error.status != null && error.status >= 400 && error.status < 500) || (!error && !data.user)) {
        console.warn(
  ...
  const value = useMemo(() => ({
    ...
    signOut: async () => { if (session?.user.id) await clearWorkerNotes(session.user.id); await supabase.auth.signOut() },
