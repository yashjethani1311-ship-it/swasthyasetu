...
export function PrescriptionsPage() {
    ...
    const [selectedPrintRx, setSelectedPrintRx] =
    ...
    function createPrescriptionPDF(
        ...
        const identityText = 'Registry verification: ' + identityLabel(doctor?.identity_source ?? null, doctor?.registry_verified ?? null)
                            ...
                            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    <button
                                        ...
                                        className="inline-flex items-center gap-2 rounded-lg bg-primary/10 border border-primary/30 px-3 py-2 text-sm font-semibold text-primary hover:bg-primary/20"
                                    >
                                        <Printer className="size-4" />

                                        {hindi
                                            ? 'A4 पर्ची (क्लीनिकल प्रारूप)'
                                            : 'Indian clinical A4 prescription layout'}
                                    </button>
                ...
                <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-xs p-4 flex justify-center items-start">
                    <div className="relative w-full max-w-4xl my-8">
                        <PrintablePrescription
                            prescription={{
                                id: selectedPrintRx.id,
                                issued_at: selectedPrintRx.issued_at,
                                status: selectedPrintRx.status,
                                clinical_notes: selectedPrintRx.clinical_notes,
                                doctor: selectedPrintRx.provider_profiles,
                                items: selectedPrintRx.prescription_items ?? []
                            }}
                            patient={{
                                id: patient?.id ?? '',
                                full_name: patient?.full_name ?? 'Patient',
                                patient_code: patient?.patient_code ?? undefined,
                                abha_number_masked: patient?.abha_number_masked ?? undefined
                            }}
