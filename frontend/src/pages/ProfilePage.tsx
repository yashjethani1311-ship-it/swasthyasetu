            ...
            <Card className="space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    <Badge tone={data?.abha_link_status === 'VERIFIED' ? 'success' : 'outline'}>
                        {data?.abha_link_status === 'VERIFIED' ? 'VERIFIED' : 'NOT_LINKED'}
                    </Badge>
                </div>
                <div className="grid gap-5 md:grid-cols-2">
                    <Field
                        ...
                        value={
                            (!data?.abha_number_masked || data.abha_number_masked.startsWith('DEMO'))
                                ? (hindi ? 'जुड़ा नहीं' : 'Not linked')
                                : data.abha_number_masked
                        }
                    />
                    <Field
                        label={hindi ? 'ABHA पता / PHR' : 'ABHA Address (PHR)'}
                        value={
                            (!data?.abha_address || data.abha_address.startsWith('DEMO'))
                                ? (hindi ? 'जुड़ा नहीं' : 'Not linked')
                                : data.abha_address
                        }
                    />
                    <Field
                        ...
                        value={data?.identity_source === 'DEMO' ? (hindi ? 'स्व-घोषित / प्रोटोटाइप' : 'Self-declared / Prototype') : (data?.identity_source ?? '—')}
                    />
                    <Field
                ...
                <div className="rounded-lg border border-border bg-surface-subtle p-3 text-xs text-muted-foreground">
                    {hindi
                        ? 'आधिकारिक ABHA / NDHM गेटवे इस वातावरण में कनेक्टेड नहीं है। डिजिटल स्वास्थ्य पहचान अभी जुड़ी नहीं है।'
                        : 'Official ABHA national health registry integration is not configured in this prototype environment. Digital health identity is currently not linked.'}
                </div>
            </Card>
