export interface VitalValidationResult<T = number> {
  value: T | null
  error: string | null
  warning?: string | null
}
...
function cleanInputString(raw: unknown, unitRegex?: RegExp): string {
...
export function normalizeVitalInteger(
  raw: unknown,
  options: NormalizeIntegerOptions,
  hindi = false
): VitalValidationResult<number> {
...
export function normalizeVitalDecimal(
  raw: unknown,
...
export function normalizeSpO2(raw: unknown, hindi = false): VitalValidationResult<number> {
...
export function normalizePulse(raw: unknown, hindi = false): VitalValidationResult<number> {
  return normalizeVitalInteger(
    raw,
    {
...
export function normalizeSystolicBP(raw: unknown, hindi = false): VitalValidationResult<number> {
  return normalizeVitalInteger(
    raw,
    {
...
export function normalizeDiastolicBP(
  raw: unknown,
...
export function normalizeTemperature(raw: unknown, hindi = false): VitalValidationResult<number> {
...
export function normalizeWeight(raw: unknown, hindi = false): VitalValidationResult<number> {
  return normalizeVitalDecimal(
    raw,
    {
      fieldName: 'Weight',
...
export function normalizeFollowUpDays(raw: unknown, hindi = false): VitalValidationResult<number> {
  return normalizeVitalInteger(
    raw,
    {
...
// Additional shared optional vitals (Requirement 5)
export function normalizeRespiratoryRate(raw: unknown, hindi = false): VitalValidationResult<number> {
  return normalizeVitalInteger(
    raw,
    {
...
export function normalizeHeight(raw: unknown, hindi = false): VitalValidationResult<number> {
  return normalizeVitalDecimal(
    raw,
    {
      fieldName: 'Height',
...
export function normalizeGlucose(raw: unknown, hindi = false): VitalValidationResult<number> {
  return normalizeVitalDecimal(
    raw,
    {
...
export interface NormalizedEncounterVitals {
...
export interface VitalsValidationOutcome {
  values: NormalizedEncounterVitals
  errors: Record<string, string>
  isValid: boolean
}

export function validateAndNormalizeEncounterVitals(
  inputs: EncounterVitalsInput,
  hindi = false
): VitalsValidationOutcome {
...
export function formatDatabaseError(error: unknown, hindi = false): string {
  const msg = error instanceof Error ? error.message : typeof error === 'string' ? error : (error as { message?: string })?.message ?? ''
