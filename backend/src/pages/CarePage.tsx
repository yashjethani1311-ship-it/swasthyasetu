...
type Snapshot = {
  outcome?: string;
  items: { source_id: string; date: string; text: string; kind: string }[];
  generated_at: string;
  notice: string;
};
        ...
        {snapshot && (
          <>
            <p className="text-sm font-semibold">
              {tr(
                "AI-selected source excerpts — verify against the original record",
            ...
            {snapshot.items.map((s, i) => (
              ...
              >
                ...
                >
                  {s.kind} · {new Date(s.date).toLocaleDateString()}
                </a>
              </article>
            ))}
            <Button
              ...
              onClick={() =>
                speakText(
                  snapshot.items.map((s) => s.text).join(". "),
                  language,
                )
              }
            >
