...
do $$declare target record; removed integer; progress integer; remaining bigint;begin
 ...
 insert into cleanup_report select 'COMMIT',true,
...
commit;
-- Fresh post-commit checks include every captured patient/provider/workflow key.
do $$declare target record; remaining bigint; n bigint;begin
 remaining:=0;
 for target in select distinct rel from cleanup_rows loop
  execute format('select count(*) from %s x join cleanup_rows r on r.rel=%s and to_jsonb(x) @> r.row_key',target.rel::regclass,target.rel) into n;
  remaining:=remaining+n;
 end loop;
 update cleanup_report set synthetic_operational_rows=remaining,
 synthetic_auth_users=(select count(*) from auth.users where lower(email) like 'swasthyasetu.test.%@example.com' or lower(email) like 'test.staging.%@example.com'),
 synthetic_facilities=(select count(*) from facilities where name like 'TEST DEMO %');
 if exists(select 1 from cleanup_report where synthetic_auth_users<>0 or synthetic_facilities<>0 or synthetic_operational_rows<>0) then raise exception 'Post-commit synthetic roots remain';end if;
end $$;
